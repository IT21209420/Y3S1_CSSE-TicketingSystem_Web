...
const GetRegisteredUser = () => {
  ...
  const { topUpAccount } = useContext(CommonContext);
  ...
  useEffect(() => {
    if (user && user.role === "stationadmin") {
  ...
  const handleTopUp = async () => {
    const errors = {
      ...
      type:
        passengerDetails.passenger.type === undefined ||
        passengerDetails.passenger.type === "",
    };
    ...
    const datas = await topUpAccount(
      Number(amountPaid),
      passengerDetails.passenger.type,
      passengerDetails.passenger._id
    );
          ...
          {passengerDetails.isGetQRClicked && (
            <div className="">
              <GeneratePermanantQrCode
                userData={passengerDetails.passenger}
